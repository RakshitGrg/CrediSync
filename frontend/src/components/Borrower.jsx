...
const Borrower = ({ expanded }) => {
  ...
  const [viewingLoanDetails, setViewingLoanDetails] = useState(false);
  ...
  const handleViewLoanDetails = (loan) => {
    setActiveLoan(loan);
    setViewingLoanDetails(true);
  };
  
  const handleBackToLoans = () => {
    setViewingLoanDetails(false);
    setActiveLoan(null);
  };
  
  const handleApplyForLoan = (loan) => {
    // Handle the loan application logic here
    console.log("Applying for loan:", loan);
    // You could navigate to an application form or submit to your API
    alert(`Application submitted for loan ${loan.loanId}`);
    setViewingLoanDetails(false);
  };
